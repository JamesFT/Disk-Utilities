    ...
    struct disk *d, unsigned int tracknr, struct stream *s)
{
    ...
    unsigned int dat_bytes = 0, gap_bits, nr_blocks = 0;
    unsigned int sec_sz;
    bool_t iam = 0, halfrate = 0;

    switch (ti->type) {
    case TRKTYP_dec_rx01:
    case TRKTYP_dec_rx02:
    case TRKTYP_mxv22m_rx01:
    case TRKTYP_mxv22m_rx02:
        halfrate = 1;
...
/* MXV22M is a dual-density third-party interface for DEC RX02 subsystem. 
 * It writes to 300RPM 5.25" DSDD disks. Sectors 1-13 on side 0, 
 * Sectors 14-26 on side 1. */
struct track_handler mxv22m_rx01_handler = {
    .density = trkden_double,
    .get_name = ibm_get_name,
    .write_raw = ibm_fm_write_raw,
    .read_raw = ibm_fm_read_raw,
...
struct track_handler mxv22m_rx02_handler = {
    .density = trkden_double,
    .get_name = ibm_get_name,
    .write_raw = ibm_fm_write_raw,
    .read_raw = ibm_fm_read_raw,
